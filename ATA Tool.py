...
def generate_coaching_summary(evaluation_record: dict, auditor_metrics: dict | pd.Series | None) -> str:
    ...
    failed = failed_df["Parameter"].dropna().tolist()
    ...
    critical_fail = failed_df["Parameter"].astype(str).str.strip().str.lower().eq("critical error identification").any()
    fail_count = int(len(failed_df))
    if risk_level == "High":
        risk_observation = "Pattern indicates elevated governance exposure and scoring inconsistency. Immediate intervention recommended."
    elif risk_level == "Moderate":
        risk_observation = "Moderate risk exposure detected. Focused coaching required to prevent escalation."
    else:
        risk_observation = "Performance within acceptable governance tolerance. Continue monitoring."
    if critical_fail:
        risk_observation = f"{risk_observation} Critical error identification failure detected in current evaluation."
    if fail_count:
        risk_observation = f"{risk_observation} Total failed controls observed: {fail_count}."
    ...
    for _, row in failed_df.drop_duplicates(subset=["Parameter", "Comment"]).iterrows():
        ...
        if comment:
            actions.append(
                f"- For '{param}', the evaluation indicates: '{comment}'. Action: Reinforce control standard, conduct focused recalibration session, and validate improvement through next two evaluations."
            )
        else:
            actions.append(
                f"- For '{param}', conduct targeted review and structured coaching with evidence-based feedback."
            )
        if param.lower() == "critical error identification":
            actions.append(
                "- Immediate governance review required. Escalate to QA leadership and conduct recalibration alignment."
            )
        ...
        if summary.empty or details.empty:
            ...
            if fig_heat:
                ...
                if not detail_fail.empty:
                    ...
                    first_fail = first_fail.rename(columns={"Parameter": "Key Failed Parameter"})
                else:
                    first_fail = pd.DataFrame(columns=["Evaluation ID", "Key Failed Parameter"])
                ...
                if interactions.empty:
                    ...
                    interactions["Reason Triggered"] = interactions.apply(_reason, axis=1)
                    out_cols = [
                        "Evaluation ID",
                        "Evaluation Date",
                        "QA Name",
